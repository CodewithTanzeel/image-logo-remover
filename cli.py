...
Usage:
    logo-remover input.jpg                          # Single file with prompt
    logo-remover --batch /path/to/folder/           # Batch processing
    logo-remover --batch /path --pattern "*.jpg"    # Batch with filter
    find . -name "*.jpg" | logo-remover --batch -   # Piping support
"""
...
def process_single(
    ...
    if success:
        ...
        print(f"[OK] Cleaned image saved to: {output_path} ({elapsed_sec:.2f}s)")
        return 0
    else:
...
Options:
  ...
  --dry-run     Preview batch without processing
    ...
    parser.add_argument(
        "--no-ask",
        action="store_true",
        help="Use default output path without prompting",
    )
